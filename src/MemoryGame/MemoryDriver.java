...
public class MemoryDriver extends GameDriver{
   ...
   // public int currentTurn = 0;  //player one goes first
    ...
    public boolean firstMove() {
        if (((MemoryPlayer)getPlayer()).getGamePiece() == null) {
            return true;
        }
        return false;
    }
    ...
    public void makeMove(int row, int col) {
        ...
        if (firstMove()) {
            ((MemoryPlayer)player).setGamePiece(card);
            //player.addPlayerCard(card);
        }
        else {
            Card otherCard = (Card)((MemoryPlayer)player).getGamePiece();
            ...
            ((MemoryPlayer)player).setGamePiece(null);
        }
    }
